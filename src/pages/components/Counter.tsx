...
interface Props {
 smartAccount: BiconomySmartAccountV2 | any;
 address: string;
...
const Counter: React.FC<Props> = ({ smartAccount, address, provider }) => {
 ...
 const handleIncrement = async () => {
  ...
  try {
   ...
   const countTx = await contract.populateTransaction.incrementCount();
 ...
 return (
  <>
   {address && (
    <button onClick={handleIncrement} className={styles.connect}>
     Increment Count
    </button>
   )}
   <ToastContainer
